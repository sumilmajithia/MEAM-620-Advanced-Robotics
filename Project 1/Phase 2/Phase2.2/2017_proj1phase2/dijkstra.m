...
startnodeindex=find(ismember(Q,start_node,'rows'),1);
...
goal_node=round(goal_node,4);
goalnodeindex=find(ismember(Q,goal_node,'rows'),1);
%%
g=inf(size(Q,1),1) ;
nodeindexarray=1:l_x*l_y*l_z;

P=zeros(numel(map),1) ; % visited node
Q_status=ones(numel(map),1);
num_expanded = 0;
indexarray=1:numel(map);
[g,indexarray,nodeindexarray]=updateheap(g,indexarray,0,startnodeindex,nodeindexarray);
n
while (Q_status(goalnodeindex)) && ((g(1))<inf)
    [g,~,index,indexarray,nodeindexarray]=extractmin(g,indexarray,nodeindexarray);   
    Q_status(index)=0;
    ...
    for s=1:numel(C)
        d=g(index)+sum(abs(Q(C(s),1:3)-Q(index,1:3)));
        if d<g(C(s))
